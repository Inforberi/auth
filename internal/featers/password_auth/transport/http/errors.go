...
func mapError(err error) ErrorMapping {
	switch {
	case errors.Is(err, password_domain.ErrInvalidEmailOrPassword):
		return ErrorMapping{
			Status:  http.StatusBadRequest,
			Code:    CodeEmailPasswordRequired,
			Message: "email or password required",
		}
	case errors.Is(err, password_domain.ErrNotFound):
		return ErrorMapping{
	...
	case errors.Is(err, password_service.ErrEmailAlreadyExists):
		return ErrorMapping{
