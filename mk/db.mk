...
db-backup:
	@mkdir -p $(BACKUP_DIR)
	$(COMPOSE) exec -T $(DB_CONTAINER) pg_dump -U $(DB_USER) $(DB_NAME) > $(BACKUP_FILE)
...
ifndef file
	...
	$(COMPOSE) exec -T $(DB_CONTAINER) psql -U $(DB_USER) $(DB_NAME) < $(file)

db-restore-replace: ## Полная замена базы
ifndef file
	$(error Укажи файл: make db-restore-replace file=backups/backup.sql)
endif
	@$(COMPOSE) exec -T $(DB_CONTAINER) psql -U $(DB_USER) $(DB_NAME) -c "DROP SCHEMA public CASCADE; CREATE SCHEMA public;"
	@$(COMPOSE) exec -T $(DB_CONTAINER) psql -U $(DB_USER) $(DB_NAME) < $(file)

db-shell: ## Открыть psql
	@$(COMPOSE) exec $(DB_CONTAINER) psql -U $(DB_USER) $(DB_NAME)